...
def test(model, data_loader, top_k, drop_flag=False, save=False):
    HR, NDCG = [], []
    u_e, i_e = model.getEmbeds()
...
def readTrainSparseMatrix(mat, degree_row, degree_col):
    row_idx, col_idx = mat.nonzero()
    row_idx = row_idx.reshape(-1,1)
    col_idx = col_idx.reshape(-1,1)
    idx = np.hstack((row_idx, col_idx)).tolist()
    length = len(idx)
    val = []
    for i in range(length):
        row, col = idx[i]
        val.append(np.sqrt(degree_row[row]*degree_col[col]))

    idx=t.LongTensor(idx)
    val=t.FloatTensor(val)
    return t.sparse.FloatTensor(idx.t(), val)
...
if __name__ == '__main__':
    ...
    d_i_train = (1/np.sum(trainMat, axis=1)).reshape(-1).A[0].tolist()
    d_j_train = (1/np.sum(trainMat, axis=0)).reshape(-1).A[0].tolist()
    # d_i_train = d_i_train.tolist()
    # d_j_train = d_j_train.tolist()

    uv_sp_mat = readTrainSparseMatrix(trainMat, d_i_train, d_j_train).cuda()
    vu_sp_mat = readTrainSparseMatrix(trainMat.T, d_j_train, d_i_train).cuda()
    ...
    model = GCCF(userNum,
                 itemNum,
                 args,
                 uv_sp_mat, vu_sp_mat, d_i_train, d_j_train).cuda()
    """
    *********************************************************
    Train.
    """
    ...
    for epoch in range(args.epoch):
        ...
        for user, item_i, item_j in train_loader:
            ...
            userEmbed, posEmbed, negEmbed = model(user, item_i, item_j)
        ...
        if epoch > 10:
            valid_hr, valid_ndcg = test(model, valid_loader, args.top_k, drop_flag=False, save=False)
        ...
        if epoch % 10 == 0 and epoch > 0:
            HR, NDCG = test(model, test_loader, args.top_k, drop_flag=False, save=False)
